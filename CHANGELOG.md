...
### Added
- Headless CLI mode: `blackbook status|list|sync|install|uninstall` run non-interactively and exit with a status code, for scripts and agents — bare `blackbook` still launches the TUI. `--tool <id>` scopes to one tool instance (real per-instance scoping for `sync`, not just a display filter); `--json` for machine-readable output; `sync` supports `--yes` (force conflicts/untracked targets) and `--dry-run`. Implemented as a thin adapter over the existing store (`useStore`/`getSyncPreview`/`syncTools`/`installPlugin`/`uninstallPlugin`) — no duplicated business logic between the TUI and CLI.
- `settings.skill_sync_mode: "copy" | "symlink"` (default `"copy"`) — opt-in symlinking for standalone-skill and plugin-component installs. A symlinked install can't drift and needs no resync. Config files are unaffected; those always stay copy-based.

### Fixed
- Local `file://` marketplace URLs were broken for plugin installs — `downloadPlugin`'s local-marketplace detection didn't recognize the `file://` scheme, and the shared path-resolution helper it now routes through had its own latent bug for that scheme. Found while verifying the symlink-sync feature against a local marketplace.
- Removed the now-obsolete `tui/scripts/patch-ink.mjs` (and its `postinstall`/`predev`/`prebuild`/`prestart` hooks). It patched an Ink v7 bug where a tall (terminal-overflowing) frame written directly to stdout left `logUpdate` internal state stale, requiring two Esc presses to close a tall detail view (see 0.24.11 below). The patch's exact-string target no longer matches current Ink source (`node_modules/ink/build/log-update.js` has been substantially rewritten upstream — cursor-position tracking, a `sync()`/`reset()` API, etc.), so it was silently no-op-ing on every install. Verified live that current Ink (7.0.4) already fixes the original bug natively — `renderInteractiveFrame` now calls `this.log.sync(outputToRender)` right after a tall write, which is exactly the fix the local patch used to bolt on by hand — so the patch is redundant, not broken-and-needing-a-rewrite.
- A `useEffect` in `App.tsx` unconditionally reset `pluginDriftMap` to `{}` on every mount (added when background drift scanning was disabled for tab responsiveness), even when a plugin detail was already open with real drift data. Discovered via 3 previously-failing (deterministic, not flaky) e2e tests that expected drift info ("Drifted"/"Changed"/line counts) to render on initial mount. Now skips the reset if `pluginDriftMap` was already populated when the effect runs. Note: this only stops the reset from clobbering pre-existing drift data — it does not restore the on-demand drift computation itself, which was removed in an earlier revert and is tracked separately as a backlog gap (plugin drift indicators don't currently populate during normal use).
