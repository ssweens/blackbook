...
/**
 * Synchronous core of `symlinkCreateModule.check` — pulled out so callers that
 * are themselves synchronous (installSkillToInstance, copyWithBackup) can use
 * it directly without an unnecessary async ripple; the module's `check` is a
 * thin async wrapper around this for interface consistency with the other
 * (genuinely async) sync modules.
 */
export function checkSymlinkSync(params: SymlinkCreateParams): CheckResult {
...
/** Synchronous core of `symlinkCreateModule.apply` — see checkSymlinkSync. */
export function applySymlinkSync(params: SymlinkCreateParams): ApplyResult {
...
export const symlinkCreateModule: Module<SymlinkCreateParams> = {
  ...
  async check(params): Promise<CheckResult> {
    return checkSymlinkSync(params);
  },
  async apply(params): Promise<ApplyResult> {
    return applySymlinkSync(params);
  },
};
