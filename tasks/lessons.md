...
- When tools have multiple instances sharing the same `toolId` (Claude + Claude Learning both use `claude-code`), **always** iterate over unique `(toolId, instanceId)` pairs from actual `installations` arrays rather than deduplicated `toolIds` or `tool.toolId` checks. The `NamespaceGroup.toolIds` field is insufficient for multi-instance tools.
- React/Ink tree UIs must use **globally unique keys** that include both `instanceId` AND skill name (e.g. `ns_uninstall_claude-code_claude-main` not just `ns_uninstall_claude-code`). Duplicate keys cause `at ink-box` errors and ghost line artifacts on expand/collapse.
- For scrollable fixed-height tree viewports, reserve space for scroll indicators (`↑ N more above` / `↓ N more below`) and use ANSI clear (`\x1b[J`) + full-width padding on every row to prevent ghost lines when content shrinks.
- Store refreshes used by user-triggered mutations must be awaited (`await refreshAll()` / `await load*()`) before returning from the action; fire-and-forget refreshes create stale-detail windows and inconsistent status badges.
- In unified action dispatch, every non-destructive mutation path must explicitly call its refresh callback (`refreshDetail*`) after the mutation promise resolves; do not rely on callers to remember per-action refresh rules.
- When validating install-state bugs, always verify both layers before declaring success: (1) real disk state (expected files/paths exist), and (2) app/store state (scanner/store reports those installs). A passing test suite alone is insufficient for state reconciliation issues.
