...
### Backlog: Restore on-demand plugin drift computation (2026-07-16 — found via patch-ink detour)
- [ ] Plugin drift indicators (per-instance "Changed"/"+N/-N" in the Installed tab and plugin detail) don't currently populate during normal use. `computePluginDrift`/`computeItemDrift` (`lib/plugin-drift.ts`, `lib/item-drift.ts`) still exist and are still exercised by unit tests, but nothing in `App.tsx` calls them anymore — confirmed via `git log -S computePluginDrift -- src/App.tsx`: added in `7ed4256`, refactored a few times, then removed by a revert (`4ed978b`, reverting `bd84365`) that appears to have dropped the on-demand recompute along with whatever else it undid. CHANGELOG's 0.24.11 entry ("Plugin drift compute is now deferred 300ms after detail open...") describes a real feature that no longer exists in the current code.
- [ ] A stale, misleading comment in `App.tsx` ("Manual-only mode... Drift is still computed on-demand when opening plugin detail") was fixed to describe current behavior accurately as part of the fix below, but the actual on-demand computation still needs to be reimplemented (deferred-after-open, bounded concurrency, matching the 0.24.11 design) — this backlog item is that follow-up.
- [x] **Immediate fix, not the full restoration** (2026-07-16): the `useEffect` that unconditionally reset `pluginDriftMap` to `{}` on every mount now skips the reset if the map was already populated when it runs — stops it from clobbering drift data that IS present (tests that seed it directly, or a future on-demand computation), without adding a new computation path itself. Found because it was making 3 e2e tests deterministically fail (`app.e2e.test.tsx`: "plugin with drift shows changed badge in list", "drifted plugin detail shows Changed instance with +/- counts", "keeps plugin drift as a per-tool row instead of a status-line badge") — confirmed pre-existing (reproduces on a clean install of unmodified HEAD), not caused by any work earlier in this session.
