...
- [ ] **Found via this verification, not fixed** (separate, real bug — same class as the `file://` marketplace-detection gap fixed earlier this session, but a different function): `plugin-drift.ts`'s private `resolvePluginSource()` only recognizes local marketplace URLs starting with `/`, `~`, `./`, `../` — a `file://` URL (a legitimate, real format; `marketplace.ts`/`path-utils.ts` both handle it explicitly elsewhere) falls through unrecognized, so drift is silently never detected for plugins from a `file://`-registered marketplace (always reports "in-sync" regardless of real state). Reproduced live while building the verification fixture for this plan; worked around by using a bare-path marketplace URL instead of fixing it, to keep this plan's scope to the drift-restoration design it was approved for.
